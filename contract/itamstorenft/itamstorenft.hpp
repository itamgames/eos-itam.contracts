...
{
    public:
        itamstorenft(name receiver, name code, datastream<const char*> ds) : contract(receiver, code, ds),
        ...
        ACTION burn(name owner, name owner_group, symbol_code symbol_name, vector<string> item_ids, string reason);
        ACTION transfernft(name from, name to, symbol_code symbol_name, vector<string> item_ids, string memo);
    ...
    private:
        ...
        void add_balance(const uint64_t symbol_raw, const name& ram_payer, const account& item);
        void sub_balance(const name& owner, const name& owner_account, const uint64_t symbol_raw, const uint64_t item_id);
};

EOSIO_DISPATCH( itamstorenft, (create)(issue)(modify)(burn)(transfernft)(receipt)(addwhitelist)(delwhitelist) )